...
from crews.react.pm_prompts import (
    PM_RETRIEVE_SYSTEM,
    PM_ANALYSE_SYSTEM,
    PM_GENERATE_SYSTEM,
)
...
def build_product_manager_graph(db_path: str = ".lancedb_pm"):
    ...
    def _react_analyse_node(state: CrewGraphState) -> dict:
        ...
        try:
            user_message = (
                f"Analyse market research for topic: {state['brief'].topic}. "
                f"Chunks available: {len(state['retrieved'])}"
            )
            _run_phase(runtime, PM_ANALYSE_TOOLS, PM_ANALYSE_SYSTEM, user_message)
            analysis = runtime.analysis
            if not analysis and state["retrieved"]:
                analysis = PMAnalysisAgent().analyse(state["retrieved"])
    ...
    def _react_generate_node(state: CrewGraphState) -> dict:
        ...
        try:
            ...
            _run_phase(runtime, PM_GENERATE_TOOLS, PM_GENERATE_SYSTEM, user_message)
            if runtime.artifacts:
                return {"artifacts": runtime.artifacts}
            from crews.product_manager.output_agent import PMOutputAgent

            artifacts = PMOutputAgent().generate_artifacts(state["analysis"], [artifact_type])
            return {"artifacts": artifacts}
        finally:
            reset_runtime(token)

    def _after_analysis(state: CrewGraphState):
        route = analysis_route(state, is_valid_pm_analysis, max_retries=2)
        if route == "continue":
            return [
